...
class Collector(mosaik_api_v3.Simulator):
    ...
    def init(
        ...
        print_results=True,
        save_h5=True,
        h5_store_name="collector_store",
        h5_frame_name="default_frame",
    ):
    ...
    def create(self, num, model, **entity_params):
        if num > 1 or self.eid is not None:
            raise RuntimeError(
                "Can only create one instance of Collector per simulator."
            )

        self.eid = "Collector"
        if self.h5_frame_name is None:
            self.h5_frame_name = self.eid
    ...
    def finalize(self):
        ...
        if self.save_h5:
            ...
            panel = pd.DataFrame(
                {
                    (unit, attribute): pd.Series(data, index=self.time_list)
                    for unit, datadict in self.data.items()
                    for attribute, data in datadict.items()
                }
            )
            # print(panel)
            print(
                "Saved to store: {0}, dataframe: {1}".format(
                    self.h5_store_name, self.h5_frame_name
                )
            )
