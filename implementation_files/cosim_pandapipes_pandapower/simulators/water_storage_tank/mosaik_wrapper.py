...
class StratifiedWaterStorageTankSimulator(Simulator):
    ...
    def init(
        ...
        time_resolution,
        step_size=10,
        eid_prefix="StratifiedWaterStorageTank",
    ):
        ...
        for eid, esim in self.simulators.items():
            ...
            for attr, incoming in data.items():
                if attr in self.input_vars:
                    if 1 != len(incoming):
                        raise RuntimeError(
                            "SimpleFlexHeatControllerSimulator does not support multiple inputs"
                        )

                    if "mdot" in attr:
                        newval = -list(incoming.values())[
                            0
                        ]  # Reverse the sign of incoming mass flow values
                    else:
                    ...
                    raise AttributeError(
                        f"StratifiedWaterStorageTankSimulator {eid} has no input attribute {attr}."
                    )
            ...
            for attr in requests:
                if attr in self.input_vars or attr in self.output_vars:
                    if attr == "T_avg":
                        ...
                        mydata[attr] = getattr(
                            esim, attr
                        )  # Convert local degK to degC for sending into the co-simulation flow
                    else:
                        mydata[attr] = getattr(esim, attr)
                else:
                    raise AttributeError(
                        f"StratifiedWaterStorageTankSimulator {eid} has no attribute {attr}."
                    )
            data[eid] = mydata
