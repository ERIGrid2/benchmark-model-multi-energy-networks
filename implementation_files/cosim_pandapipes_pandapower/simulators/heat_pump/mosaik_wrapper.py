        ...
        for eid, esim in self.simulators.items():
            ...
            for attr, incoming in data.items():
                if attr in self.input_vars:
                    if 1 != len(incoming):
                        raise RuntimeError(
                            "ConstantTcondHPSimulator does not support multiple inputs"
                        )
                    ...
                    raise AttributeError(
                        f"ConstantTcondHPSim {eid} has no input attribute {attr}."
                    )
        ...
        for eid, esim in self.simulators.items():
            ...
            for attr in requests:
                if attr in self.input_vars or attr in self.output_vars:
                    ...
                    raise AttributeError(
                        f"ConstantTcondHPSim {eid} has no attribute {attr}."
                    )
